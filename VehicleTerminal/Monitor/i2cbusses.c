...
void free_adapters(struct i2c_adap *adapters)
{
    ...
    for (i = 0; adapters[i].name; i++) free(adapters[i].name);
    free(adapters);
}
...
{
    const struct i2c_adap *adap1 = (const struct i2c_adap *)a;
    const struct i2c_adap *adap2 = (const struct i2c_adap *)b;
...
struct i2c_adap *gather_i2c_busses(void)
{
    ...
    while ((de = readdir(dir)) != NULL) {
        ...
        if (f == NULL) {
            len = snprintf(n, NAME_MAX, "%s/%s/device/name", sysfs, de->d_name);
            if (len >= NAME_MAX) {
                fprintf(stderr, "%s: path truncated\n", n);
                continue;
            }
        ...
        if (f == NULL) {
            len = snprintf(n, NAME_MAX, "%s/%s/device", sysfs, de->d_name);
            if (len >= NAME_MAX) {
                fprintf(stderr, "%s: path truncated\n", n);
                continue;
            }
            if (!(ddir = opendir(n))) continue;
            while ((dde = readdir(ddir)) != NULL) {
                if (!strcmp(dde->d_name, ".")) continue;
                if (!strcmp(dde->d_name, "..")) continue;
                if ((!strncmp(dde->d_name, "i2c-", 4))) {
                    len = snprintf(n, NAME_MAX, "%s/%s/device/%s/name", sysfs, de->d_name, dde->d_name);
                    if (len >= NAME_MAX) {
                        fprintf(stderr, "%s: path truncated\n", n);
                        continue;
                    }
                    if ((f = fopen(n, "r"))) goto found;
                }
            }
        }
    ...
    found:
        if (f != NULL) {
            ...
            if ((px = strchr(s, '\n')) != NULL) *px = 0;
            if (!sscanf(de->d_name, "i2c-%d", &i2cbus)) continue;
            if (!strncmp(s, "ISA ", 4)) {
                type = adt_isa;
            } else {
...
int open_i2c_dev(int i2cbus, char *filename, size_t size, int quiet)
{
    ...
    if (file < 0 && !quiet) {
        if (errno == ENOENT) {
            ...
            if (errno == EACCES) fprintf(stderr, "Run as root?\n");
        }
    }
